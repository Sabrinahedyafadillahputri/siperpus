...
if(isset($_POST['btnPinjam'])){
	...
	$id_ptgs            = $_SESSION['id_ptgs'];
 
	

	$sql = "INSERT INTO peminjaman(id_anggota, tgl_pinjam, tgl_jatuh_tempo, id_ptgs) VALUES('$id_anggota', '$tgl_pinjam', '$tgl_jatuh_tempo', '$id_ptgs')";
	
	$stok = ambilStok($kon, $id_buku); //ambil data stok buku

	if(cekPinjam($kon, $id_anggota) && $stok > 0){
		$res = mysqli_query($kon, $sql);
		$querp=mysqli_query($kon,"SELECT id_pinjam FROM peminjaman WHERE tgl_pinjam='$tgl_pinjam' AND id_anggota='$id_anggota' AND tgl_jatuh_tempo='$tgl_jatuh_tempo' AND id_ptgs='$id_ptgs' ");
